/* Using subqueries (inner and alter query) with functions */
...
SELECT NOME, MARCO AS MENOR_MARCO
FROM VENDEDORES
WHERE MARCO = (SELECT MIN(MARCO) FROM VENDEDORES);

SELECT NOME, MARCO AS MAIOR_MARCO
FROM VENDEDORES
WHERE MARCO = (SELECT MAX(MARCO) FROM VENDEDORES);

SELECT NOME, FEVEREIRO AS ACIMA_MEDIA_FEV
FROM VENDEDORES
WHERE FEVEREIRO > (SELECT AVG(FEVEREIRO) FROM VENDEDORES);

SELECT NOME, JANEIRO AS ABAIXO_MEDIA_JAN
FROM VENDEDORES
